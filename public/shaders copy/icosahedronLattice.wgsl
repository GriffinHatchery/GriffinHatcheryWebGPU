...
fn quaternionRotation(v: vec3<f32>, q: vec4<f32>) -> vec3<f32> {
    return quaternionMult(quaternionMult(quaternionInv(q), vec4<f32>(v, 0.0)), q).xyz;
}
...
fn vertex_main(@location(0) position: vec3<f32>, @location(1) norm: vec3<f32>, @builtin(instance_index) instance_index: u32) -> VertexOut {
    ...
    let a = (center.x + center.y + 1.46 * center.z) * 0.35;
    let b = (1.7 * center.x + center.y + center.z) * 1.52342;

    let rotp = rotx(rotz(position, a), b);
